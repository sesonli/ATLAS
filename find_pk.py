...
def read_pdb_and_find_nt(pdb_filepath, node_id):
    '''Extract atomic coordinates for a specific nucleotide from PDB file'''
...
def extract_node_info(node_id, pdb_filename):
    '''Extract atomic coordinates for nodes from PDB files'''
    pdb_filename_base = pdb_filename[:4]  
    correct_pdb_filename = pdb_filename_base + '.pdb'
    pdb_filepath = os.path.join(pdb_folder, correct_pdb_filename)
...
def create_and_populate_db(data):
    conn = sqlite3.connect("data/ATLAS20250703_pk.db")
    cursor = conn.cursor()
    cursor.execute('DROP TABLE IF EXISTS PK')
    cursor.execute('''CREATE TABLE IF NOT EXISTS PK (
    ...
    for entry in data:
        for pdbid, kps in entry.items():
            for kp_key, kp_data in kps.items():
                ...
                pdbid_r = pdbid[:4]  # Extract 4-letter PDB ID
                
                # Extract filecontent for all nodes
                combined_filecontent = []
                for node in nodes:
                    filecontent = extract_node_info(node, pdbid)
                    combined_filecontent.append(filecontent)
                combined_filecontent_str = '\n'.join(combined_filecontent)
                
                if "." not in nt_number:  # Skip entries with missing data
                    cursor.execute('''INSERT INTO PK (motif_type, pdbid, paired_nt_number, nt_number, filecontent, dot_bracket)
                                  VALUES (?, ?, ?, ?, ?, ?)''', 
                                  (motif_type, pdbid_r, paired_nt_number, nt_number, combined_filecontent_str, dot_bracket))

    conn.commit()
    conn.close()

with open("data/saved_graphs_canonical_new.pickle", "rb") as f:
    graph_data = pickle.load(f)
import os
pdb_folder = r"data\rna_chain_corrected_2"
DNA_complex = []
for filename in os.listdir(pdb_folder):
    if filename.endswith(".pdb"):  # Check if the file has the correct extension
        ...
        if any(nucleotide in content for nucleotide in [" DA ", " DU ", " DG ", " DC "]):
            ...
            key = filename.replace(".pdb", "_annotation.txt")
...
for key, graph in graph_data.items():
    ...
    count += 1
    # Process all RNA structures (no limit)
